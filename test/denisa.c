...
void *f(void *arg)
{
    ...
    //fiecare thread poate calcula mai multe Mi daca P < 7
    for( int i = thread_id; i< 7; i+=P) { //i = operatia Mi pe care o face thread ul 
        switch(i) {
    ...
    // sincronizare simplă: doar thread-ul 0 face combinarea și print-ul
    if(thread_id == 0) {
        // submatricile finale
...
int main(int argc, char *argv[])
{
    ...
    {
        ids[i] = i;
        if(pthread_create(&threads[i], NULL, f, &ids[i])) {
            perror("pthread create estuat");
            exit(-1);
        }
    }
